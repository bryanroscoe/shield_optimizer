...
.PHONY: test test-verbose test-coverage fixtures lint help connect clean screenshots screenshots-auto screenshots-gif screenshots-all
...
help:
	...
	@echo "    screenshots-all  - Clean, capture PNGs, and create GIF (one command)"
...
screenshots-auto:
	...
	@i=1; for name in $(SCREENSHOT_NAMES); do \
		echo "Capturing $$name..."; \
		termshot --no-shadow --columns 70 -f screenshots/$$name.png -- pwsh -NoProfile -File ./demos/ScreenshotGallery.ps1 -Screen $$i; \
		i=$$((i + 1)); \
	done
	@echo "Done! Screenshots saved to ./screenshots/"

# Create animated GIF from screenshots (requires: ffmpeg)
# Uses two-pass palette generation for quality, 1 second per frame
screenshots-gif:
	@echo "Creating animated GIF (1s per frame)..."
	@mkdir -p screenshots/tmp
	@for name in $(SCREENSHOT_NAMES); do \
		cp screenshots/$$name.png screenshots/tmp/$$name.png; \
	done
	@printf "%s\n" \
	...
	@cd screenshots/tmp && ffmpeg -y -f concat -i concat.txt \
		-vf "split[s0][s1];[s0]palettegen=reserve_transparent=1:stats_mode=diff[p];[s1][p]paletteuse=dither=floyd_steinberg:alpha_threshold=128" \
		-gifflags +transdiff -loop 0 ../gallery.gif 2>/dev/null
	@rm -rf screenshots/tmp
	@echo "Done! Created screenshots/gallery.gif ($$(du -h screenshots/gallery.gif | cut -f1))"

# All-in-one: clean, capture, and create GIF
screenshots-all:
	@echo "Cleaning screenshots folder..."
	@rm -rf screenshots/*.png screenshots/*.gif screenshots/tmp
